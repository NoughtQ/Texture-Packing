...
int level = 0;                             // Current level of the resulting texture

int cmp(const void * a, const void * b);   // Comparator for decrease-order quicksort function
void printDebugInfo(int n, Item * rect);   // Print the debug infomation when using "--debug" command argument
...
int cmp(const void * a, const void * b) {
    ...
    return dataB.height - dataA.height;    // Beware of the order of subtraction!
}
